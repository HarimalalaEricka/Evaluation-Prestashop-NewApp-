...
export async function upsertStockAvailable({ productId, productAttributeId = '0', quantity, idShop, idShopGroup, outOfStock, dependsOnStock }) {
    ...
    try {
        ...
        if (stock?.id) {
            ...
            // CREATE new
            const xml = `<?xml version="1.0" encoding="UTF-8"?>\n<prestashop xmlns:xlink="http://www.w3.org/1999/xlink">\n<stock_available>\n<id_product><![CDATA[${cleanedProductId}]]></id_product>\n<id_product_attribute><![CDATA[${cleanedProductAttributeId}]]></id_product_attribute>\n<quantity><![CDATA[${cleanedQuantity}]]></quantity>\n<id_shop><![CDATA[${cleanedIdShop}]]></id_shop>\n<id_shop_group><![CDATA[${cleanedIdShopGroup}]]></id_shop_group>\n<out_of_stock><![CDATA[${cleanedOutOfStock}]]></out_of_stock>\n<depends_on_stock><![CDATA[${cleanedDependsOnStock}]]></depends_on_stock>\n</stock_available>\n</prestashop>`
            return await insertResourceData('stock_availables', xml)
        }
    } catch (err) {
...
export async function insertCombinationWithStock({ productId, reference, price, optionValueId, defaultOn, stockQuantity, languageIds = [1] }) {
    // 1. Crée la combinaison
    const xml = buildCombinationXml({ productId, reference, price, optionValueId, defaultOn })
    const response = await insertResourceData('combinations', xml)

    // 2. Récupère l'ID de la combinaison créée
    const combinationId = extractCreatedId(response)

    if (!combinationId) {
        throw new Error(`Combinaison créée mais ID introuvable pour produit ${productId}`)
    }

    // 3. Met à jour le stock via stock_availables avec id_product_attribute = combinationId
    if (stockQuantity !== null && stockQuantity !== undefined) {
        try {
            await upsertStockAvailable({
                productId,
                productAttributeId: combinationId,  // ← clé : l'ID de la combination
                quantity: stockQuantity,
            })
        } catch (err) {
            console.warn(`[STOCK] Stock non mis à jour pour combinaison ${combinationId}:`, err.message)
        }
    }
